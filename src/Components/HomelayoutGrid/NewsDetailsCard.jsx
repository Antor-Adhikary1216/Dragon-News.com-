...
const NewsDetailsCard = ({news}) => {
    ...
    return (
        <div>
             <div className="main-card   mt-5 p-7 shadow-md ">
                <img src={image_url} alt="" className='mx-auto rounded-md' />
                <h1 className='my-5 font-bold text-xl'>{title}</h1>
        
                <p className='text-[#706F6F] '>{details}</p>
                    <NavLink to={`/cetagory/${category_id}`}> <button className="btn my-5 bg-[#D72050] text-white  btn-xs sm:btn-sm text-center md:btn-md lg:btn-lg xl:btn-xl"> <FaArrowLeft /> All news in this category</button></NavLink>
